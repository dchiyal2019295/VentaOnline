...
function BuscarporDefecto(req,res) {
    Categoria.findOne({nombre: 'Default'}).exec((err,categoriaEncontrado)=>{
        if(err) return res.status(500).send({mensaje:'Error en la peticion'})
        if(!categoriaEncontrado) console.log('El producto que busca no existe')
        if(categoriaEncontrado) console.log('categoria default encontrado')
        idDef = categoriaEncontrado;
...
function EliminarCategoria(req, res) {
    ...
    if(req.user.rol != 'ROL_ADMIN'){
        return res.status(500).send({mensaje: 'No posee el permiso para eliminar '})
    }

    Categoria.findOne({_id: idCategoria}).exec((err,categoriaEncontrada)=>{
        if(err) return res.status(500).send({mensaje:'Error en la peticion'})
        if(!categoriaEncontrada) return res.status(500).send({mensaje:'Los datos no existen'})
        if(categoriaEncontrada.nombre === 'Default') return res.status(500).send({mensaje:'No posee permiso para eliminar La Categoria Por Defecto'})
